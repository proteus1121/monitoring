...
export const UsersPage = () => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return (items ?? []).filter(i => i.authorities?.includes(filter));
  }, [items]);
          ...
          {filteredItems.map(i => (
            <Card className="flex" key={i.id}>
              <div className="flex flex-1 items-center gap-4">
                <div
                  className={cn(
                    'grid aspect-square w-12 place-items-center rounded-full bg-current/20 p-3',
                    {
                      'text-blue-500': true,
                    }
                  )}
                >
                  {i.username && i.username.length > 1 ? (
                    <div>{i.username.slice(0, 2).toUpperCase()}</div>
                  ) : (
                ...
                <span
                  className={`rounded-full bg-yellow-500 px-2 text-xs text-white`}
                >
                  {JSON.stringify(i.authorities)}
                </span>
              </div>
